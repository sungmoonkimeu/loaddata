...
time = pd.to_numeric(data['Index']) / 10 / 60 - 9000/10/60
...
draw_stokes_points(fig2[0], Out[9000:-1], kind='line', color_line=cstm_color[0])

azi_V = Out[9000:-1].parameters.azimuth()
ellip_V = Out[9000:-1].parameters.ellipticity_angle()
diff_azi_V = azi_V[0:1000].max() - azi_V[0:1000].min()
diff_ellip_V = ellip_V.max() - ellip_V.min()

ax[0].plot(time[9000:-1], S0[9000:-1])
# ax[0].set(xlim=(0, 0.5), ylim=(-1, 1))
ax[1].plot(time[9000:-1], S1[9000:-1])
# ax[1].set(xlim=(0, 0.5), ylim=(-1, 1))
ax[2].plot(time[9000:-1], S2[9000:-1])
# ax[2].set(xlim=(0, 0.5), ylim=(-1, 1))
ax[3].plot(time[9000:-1], S3[9000:-1])
# ax[3].set(xlim=(0, 0.5), ylim=(-1, 1))

fig3, ax3 = plt.subplots(figsize=(6, 5))
ax3.plot(time[9000:-1], (azi_V-azi_V[0])*180/pi, label="azimuth")
ax3.plot(time[9000:-1], (ellip_V-ellip_V[0])*180/pi, label="ellipticity")
