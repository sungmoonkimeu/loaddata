...
if __name__ =='__main__':
    ...
    # SOP change
    fig2, ax2 = plt.subplots(figsize=(8.5/2.54, 7/2.54))
    ...
    #fig3, ax3 = Sv.draw_poincare(figsize=(7, 7), angle_view=[0.2, 1.2], kind='line')
    ...
    if os.path.splitext(file_list[0])[0].split('_')[0][2:] == 'Hz':
        ...
        ax2.set_ylabel('SOP change (deg)')
        ax2.set(xlim=(9.5, 30.5), ylim=(0, 2))
    else:
        ax2.plot(ang_SOP, alpha * 180 / pi,'k')
        #ax2.plot(ang_SOP, diff_azi_V * 180 / pi)
        #ax2.plot(ang_SOP, diff_ellip_V * 180 / pi)
        ax2.set_xlabel('Azimuth angle of input SOP (deg)')
        ax2.set_ylabel('SOP change (deg)')
        ax2.set(xlim=(0, 360), ylim=(0, 2))
    '''
