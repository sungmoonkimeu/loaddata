...
maxdatay1 = np.array([])
maxdatay2 = np.array([])
maxdatay1_err_upper = np.array([])
maxdatay1_err_lower = np.array([])
maxdatay2_err_upper = np.array([])
...
for nn, fn in enumerate(list_fn):
    ...
    #print(np.fft.fftfreq(len(fdata))*fs)
...
for nn in yy:
    ...
    maxdatay1_err_upper = np.append(maxdatay1_err_upper, 1/xdata[nn-1])
    maxdatay1_err_lower = np.append(maxdatay1_err_lower, 1 / xdata[nn + 1])
...
for nn in yy2:
    ...
    maxdatay2_err_upper = np.append(maxdatay2_err_upper, 2 / xdata[nn - 1])
    maxdatay2_err_lower = np.append(maxdatay2_err_lower, 2 / xdata[nn + 1])
...
Lb_e = 2*pi/sqrt((2*pi/LB2)**2 + 4*(STR0 - (1-g)*STR)**2)

ax2.errorbar(maxdatax1, maxdatay1, yerr=[maxdatay1 - maxdatay1_err_lower, maxdatay1_err_upper-maxdatay1], ls="None", c='black', ecolor='lightgray', elinewidth=3, capsize=6)
ax2.scatter(maxdatax1, maxdatay1, label="Measured (FFT 1st peak)")
...
ax2.scatter(maxdatax2, maxdatay2, label="Measured (FFT 2nd peak)")

ax2.plot(twist, Lb_c, label="Simulated (LB=0.25 m)")
ax2.plot(twist, Lb_d, label="Simulated (LB=0.30 m)")
ax2.plot(twist, Lb_e, label="Simulated (LB=0.35 m)")
